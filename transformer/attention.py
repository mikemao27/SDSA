...
class SpikingSelfAttention(nn.Module):
    ...
    {0.0, 1.0}). Split Q, K, V across num_heads attention heads and combine them into an attention output (e.g. (Q @ K^T) @ V, scaled by attn_scale). 
    Merge the heads, project back to embed_dim, and pass the result through a final output LIF neuron.
    ...
    with V (analogous to the 1 / sqrt(d_k) scaling used in standard softmax attention).
    """
    def __init__(
        self,
        embed_dim: int,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for batched matmul. We compute attention scores with a batched matmul over the trailing two dims (torch.matmul batches over all leading dims 
        automatically). We combine it with V and merge the heads back. Lastly, we project it back to embed_dim and spike once more, and return the output.
        """
        T, B, N, C = x.shape
