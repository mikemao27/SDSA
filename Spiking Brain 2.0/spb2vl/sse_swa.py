...
from fla.modules import FusedRMSNormGated, ShortConvolution, RotaryEmbedding#, RMSNorm
...
class GDNFP32Params(nn.Module):
    def __init__(self, n: int):
        super().__init__()
        A = torch.empty(n, dtype=torch.float32).uniform_(0, 16)
        ...
        dt = torch.exp(torch.rand(n) * (math.log(dt_max) - math.log(dt_min)) + math.log(dt_min))
        dt = torch.clamp(dt, min=dt_init_floor)
        inv_dt = dt + torch.log(-torch.expm1(-dt))
        self.dt_bias = nn.Parameter(inv_dt.to(torch.float32))
        self.dt_bias._no_weight_decay = True
    
    def forward(self, dummy_input: torch.Tensor = None):
...
class SSEGDNH(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert num_reader <= num_sparse_partition and num_writer <= num_sparse_partition, \
            "num_reader and num_writer must be less than or equal to num_sparse_partition."
        assert sse_implementation in ["mask", "varlen"], \
            f"Unknown SSE implementation {sse_implementation}"
        ...
        # self.dt_bias._no_weight_decay = True
        self.fp32_params = GDNFP32Params(self.sse_num_kv_heads*2)
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        if self.allow_neg_eigval:
            b = b * 2.
        A_log, dt_bias = self.fp32_params(hidden_states)
        g = -A_log.float().exp() * F.softplus(self.sse_a_proj(hidden_states).float() + dt_bias)
        ...
        if self.swa_qk_norm:
            q, k = self.swa_q_norm(q), self.swa_k_norm(k)
            # swa_softmax_attention only supports bfloat16 or fp16
        ...
        o = (self.sse_merge_norm(sse_o) + self.swa_merge_norm(swa_o)) / 2
        # o = (swa_o + self.sse_merge_norm(sse_o)) / 2
